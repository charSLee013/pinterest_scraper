...
def download_images_with_cache(
    ...
    for i, pin in enumerate(pins):
        ...
        # 将描述限制在20个字符以内，避免文件名过长
        short_desc = utils.sanitize_filename(description[:20] if description else "")

        # 从URL获取标识
        url_hash = ""
        if largest_url:
            url_parts = largest_url.split("/")
            if url_parts and len(url_parts) > 1:
                url_hash = url_parts[-1].split(".")[0][:8]  # 取最后部分的前8个字符

        # 组合文件名，确保有描述和URL信息
        if short_desc and url_hash:
            filename = f"{safe_prefix}_{pin_id}_{short_desc}_{url_hash}.jpg"
        elif short_desc:
            filename = f"{safe_prefix}_{pin_id}_{short_desc}.jpg"
        elif url_hash:
            filename = f"{safe_prefix}_{pin_id}_{url_hash}.jpg"
        else:
