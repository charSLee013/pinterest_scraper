...
### 🔧 数据库架构统一 (v4.2.1)
- **Hash算法统一**: 修复第二阶段UNIQUE constraint failed错误，确保数据完整性
- **GraphQL数据优化**: 完善v3RelatedPinsForPinSeoQuery解析，支持entityId和imageSpec_*字段
- **架构一致性**: 第一阶段和第二阶段使用统一的标准化数据保存流程
- **Pin验证增强**: 支持GraphQL格式的图片字段验证，提高数据质量
- **冲突预防机制**: 智能检测重复Pin，避免数据库约束冲突

## 🆕 v4.2 新增功能

### 🚀 多核加速转换器 (v4.2.0)
- **多线程并发处理**: 充分利用多核CPU，显著提升Base64转换效率
- **智能线程管理**: 自动检测CPU核心数，默认配置最优工作线程数
- **保持原子性**: 每个Pin仍保持独立的原子事务，确保数据一致性
- **优雅中断支持**: 支持Ctrl+C随时中断，已处理数据安全保存
- **Windows兼容进度条**: 真正的单行进度显示，不再多行刷屏
- **性能提升**: 大数据量场景下转换速度提升4倍（90+ pins/秒）

## 🆕 v4.1 新增功能

### 📥 仅下载图片功能 (--only-images)
- **智能图片补全**: 从现有数据库中检测并下载缺失的图片文件
- **单关键词模式**: `--only-images --query cats` 仅下载指定关键词的图片
- **全关键词模式**: `--only-images` 下载所有关键词的缺失图片
- **智能跳过**: 自动跳过已存在且有效的图片文件（>1KB）
- **完善验证**: 严格的参数验证，防止与其他选项冲突

### 🔄 数据库合并工具 (merge_databases.py)
- **独立合并脚本**: 专门的数据库合并工具，支持多数据源整合
- **目标优先策略**: 相同Pin ID时保留目标数据库的数据，避免数据丢失
- **预览模式**: `--dry-run` 参数预览合并操作，确保安全
- **智能发现**: 自动扫描关键词目录，发现可合并的数据库
- **批量处理**: 高效的批量数据处理，支持大规模数据库合并
- **详细报告**: 完整的合并统计和进度报告

### 🚨 实时保存架构重构 (v4.1.2)
- **彻底重构数据流**: 解决了数据采集与存储脱节的根本问题
- **消除内存临时存储**: 每采集到一个Pin立即保存到数据库，零数据丢失
- **恶劣环境适配**: 优先数据可靠性而非性能，适合频繁中断的运行环境
- **真正实时保存**: 进度条基于实际数据库保存量，确保完全同步
- **强化中断保护**: 中断时所有已采集数据都已安全保存，100%可靠

### ⚡ 性能优化架构 (v4.2.0)
- **多核Base64转换**: 自动利用多核CPU加速Pin ID转换处理
- **批量数据库操作**: 优化数据库查询和更新策略，减少I/O开销
- **智能进度显示**: Windows兼容的单行进度条，清晰显示处理状态
- **线程安全设计**: 每线程独立数据库连接，确保并发安全
- **自动性能调优**: 根据系统配置自动选择最优并发参数
...
### 💾 实时保存架构 (v4.1.2重构)
- **真正实时保存**: 采集过程中每个Pin立即保存到数据库，无内存缓存
- **恶劣环境优化**: 专为频繁中断环境设计，数据可靠性优先于性能
- **零数据丢失**: 中断时所有已采集数据都已安全保存在数据库中
- **进度同步**: 进度条显示实际数据库保存量，确保数据一致性
- **基于数据库的去重**: 利用SQLite UPSERT机制高效处理重复数据

### 📥 --only-images 四阶段工作流程 (v4.2.0重构)
- **阶段1**: 数据库修复与检测 - 自动检测并修复损坏的数据库文件
- **阶段2**: Base64编码Pin转换 - 将base64编码转换为真实Pin ID
- **阶段3**: Pin详情数据补全 - 批量获取缺失的Pin详情信息
- **阶段4**: 图片文件下载 - 5步循环逻辑高效下载
  - 步骤1: 创建已下载图片pin集合
  - 步骤2: 翻页批量读取待下载pins
  - 步骤3: 检查/获取headers
  - 步骤4: 多线程下载图片（重试+容错）
  - 步骤5: 检查本地文件并更新全局计数
- **批次优化**: batch_size自动等于max_concurrent，资源利用率100%
...
```bash
# 下载指定关键词的缺失图片
python main.py --only-images --query cats
...
# 开始下载关键词图片: cats
# 发现 1 个关键词数据库
# 关键词 cats: 发现 15 个缺失图片
# 下载 cats: 100%|████████████| 15/15 [00:12<00:00, 1.25img/s]
# 下载完成: 15 成功, 0 失败
```
...
```bash
# 预览合并操作（推荐先预览）
python merge_databases.py --source main_output --target additional_data --dry-run

# 执行实际合并（将additional_data合并到main_output中）
python merge_databases.py --source main_output --target additional_data --verbose

# 输出示例：
# 数据库合并预览
# ============================================================
# cats: 150 pins (合并到现有)
# dogs: 80 pins (新建到主数据库)
# 总计: 2 个关键词
# 主数据库: 100 pins
# 要合并的数据: 230 pins
# 注意: 主数据库中的数据将优先保留（相同Pin ID时）
# ============================================================
# 是否继续合并? (y/n): y
#
# 处理关键词: cats
# 合并 cats: 100%|████████████| 50/50 [00:02<00:00, 25.0pins/s]
# 关键词 cats 完成: 合并 50, 跳过 100
#
# 数据库合并完成!
# 处理关键词: 2 个
# 合并Pin: 130 个
...
**重要说明**：
- `--source`: 主数据库目录（合并后的最终位置）
- `--target`: 要合并的数据库目录（数据来源）
- 合并方向：将target的数据合并到source中
- 冲突处理：相同Pin ID时，保留source（主数据库）中的数据
...
- **原子化数据保存**: INSERT OR REPLACE策略确保数据一致性，完全解决SQLAlchemy UPSERT错误
- **鲁棒性设计**: 可选字段缺失不影响保存成功，最新数据自动覆盖旧数据
...
- Python 3.10+
- Windows/Linux/macOS
- **重要**: 项目依赖brotli库处理Pinterest的压缩响应，确保多线程Pin详情获取功能正常工作
...
#### 故障排除：多线程Pin详情获取失败
如果遇到多线程Pin详情获取返回乱码或失败，通常是缺少brotli库支持：

**症状**：
- HTML内容显示为乱码二进制数据
- Pin详情提取失败，成功率为0%
- 日志显示"未能从HTML中提取到Pin数据"
...
```bash
# 运行多线程测试
uv run python test_multithread_pin_fetch.py
# 应该显示100%成功率和正确的HTML内容
```
...
| 参数 | 说明 | 默认值 |
|------|------|--------|
| `--only-images` | 仅下载图片模式：从现有数据库中下载缺失图片 | False |
...
### 数据库合并工具参数 (merge_databases.py)
| 参数 | 说明 | 默认值 |
|------|------|--------|
| `--source` | 主数据库目录路径（合并后的最终位置） | - |
| `--target` | 要合并的数据库目录路径（数据来源） | - |
| `--dry-run` | 预览模式，不实际执行合并 | False |
| `--verbose` | 详细输出模式 | False |
| `--debug` | 调试模式 | False |
